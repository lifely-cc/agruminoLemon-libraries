/*
  AgruminoDweet.ino - Sample project for Agrumino board using the Agrumino
  library.
  ...
  This sketch reads every 1h all values from the Arduino board
  and update them to the Dweet.io service every 4h.
...
void loop() {
  ...
  // repeatedly
  agrumino.deepSleepSec(SLEEP_TIME_SEC); // ESP8266 enter in deepSleep and after
                                         // the selected time starts back from
                                         // setup() and then loop()
}
...
// settings.
// If "checkBattery" is true the method return true only if the USB is
// connected.
